...
public:
	...
	double sum_after_a_time() //функция подсчёта конечной суммы вклада
	{
		double temp = sum;
	...
	void virtual print1() //функция печати для демонстрации динамического полиморфизма
	{
		cout << "ФИО: " << FIO << endl;
		cout << "Начальная сумма счёта: " << sum << endl;
		cout << "Ставка вклада(в %): " << deposit_rate << endl;
...
public:
	...
	void print1() //функция печати для демонстрации динамического полиморфизма
	{
		Bank::print1();
		cout << "Процент кредита(в %): " << credit << endl;
	}
...
int main()
{
	...
	double Sum1 = contrib->sum_after_a_time(); //сумма вклада после начисления всех %
	contrib->print(); //демонстрация статического полиморфизма
	cout << "Сумма по окончании вклада (обычный счёт): " << Sum1 << endl << endl;
	preferred_Bank* contrib_pref = new preferred_Bank("FIO", 1000, 10, 3, 20); //создание указателя на производный класс с начальными значениями
	double Sum2 = contrib_pref->sum_after_a_time(); //сумма вклада с кредитом после начисления всех %
	contrib_pref->print1(); //демонстрация динамического полиморфизма
