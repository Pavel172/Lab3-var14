...
int main()
{
	...
	double Sum1 = contrib->sum_after_a_time(); //сумма вклада после начисления всех % и демонстрация динамического полиморфизма (функция в классе описана с модификатором virtual)
	contrib->print(); //демонстрация статического полиморфизма
	cout << "Сумма по окончании вклада (обычный счёт): " << Sum1 << endl << endl;
	Bank* contrib_pref = new preferred_Bank("FIO", 1000, 10, 3, 20); //создание указателя на производный класс с начальными значениями
	double Sum2 = contrib_pref->sum_after_a_time(); //сумма вклада с кредитом после начисления всех %
	contrib_pref->print(); //т.к. функция print описана в классе без модификатора virtual, то в данном случае вызывается функция print базового класса
